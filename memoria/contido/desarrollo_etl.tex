...
Para no sobrecargar la base de datos transaccional durante los cruces analíticos, el primer paso es clonar las tablas de producción al esquema intermedio \textit{raw} (Staging Area). El \textit{pipeline} establece 14 hilos independientes que extraen datos de forma incremental usando el parámetro \textit{\$\{CURSO\_ACTUAL\}}, permitiendo recargas históricas completas si el parámetro se omite. 
...
El núcleo de las reglas de negocio reside en este \textit{pipeline}, cuya misión es ingerir datos brutos de encuestas y construir una dimensión de baja cardinalidad consolidando perfiles de riesgo socioeconómico (ver figura \ref{fig:dim_demografia_familiar}).
...
Este sistema requiere una dimensión de tiempo académica que categorice momentos evaluativos ("Curso 2023-2024, 1ª Evaluación"). El \textit{pipeline} infiere automáticamente estas temporalidades desde los registros transaccionales de matrículas y expedientes (Figura \ref{fig:dim_tiempo}).
...
El registro transaccional de adaptaciones suele ser texto libre. Este \textit{pipeline} actúa como motor de extracción de reglas clínicas mediante expresiones regulares (\textit{RegexEval}), clasificando el texto en discapacidades fisiológicas/neuropsicológicas (TDAH, Dislexia, etc.) o necesidades compensatorias sociológicas (absentismo, acogimiento, etc.). Cada tipología suma puntos de severidad (10 o 5) para crear la métrica unificada \textit{riesgo\_adaptacion} (ver figura \ref{fig:dim_adaptacion}).
...
Los \textit{pipeline}s son secuenciados por flujos de trabajo (\textit{Workflows}). La orquestación sigue una arquitectura jerárquica encabezada por \textit{main.hwf} (ver figura \ref{fig:main_workflow}), que delega en dos sub-workflows:
...
\begin{itemize}
    ...
    \item \textbf{\textit{facts.hwf}:} Ejecuta secuencialmente las tablas de hechos. Un nodo \textit{Simple Evaluation} inspecciona el parámetro dinámico \textit{\$\{CURSO\_ACTUAL\}}: si está vacío, enruta hacia el \textit{pipeline} \textit{Full}; si contiene un curso, enruta hacia el \textit{Incremental} (ver figura \ref{fig:facts_workflow}).
\end{itemize}

El parámetro \textit{CURSO\_ACTUAL} fluye en cascada desde la ejecución externa (Docker) hasta el código SQL de los \textit{pipeline}s, permitiendo que todo el ecosistema adapte su comportamiento a un nuevo año académico alterando un único valor.

La robustez se garantiza mediante un manejo de errores multinivel: si un \textit{pipeline} individual falla, el sub-workflow detiene la ejecución, registra el error y lanza un \textit{Abort workflow} que se propaga jerárquicamente hasta abortar la orquestación global, previniendo cargas parciales o corruptas.
