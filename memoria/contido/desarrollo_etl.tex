...
Una decisión clave es la \textbf{ausencia de claves foráneas} en el esquema destino \textit{raw}, lo que permite a Hop paralelizar la inserción masiva de todas las entidades simultáneamente sin generar bloqueos lógicos por dependencias (Figura \ref{fig:extract_all_to_staging}).
...
La casuística de un estudiante no es estática: puede cambiar de domicilio o tutor legal a lo largo de los años. Para no perder el contexto socioeconómico histórico asociado a calificaciones pasadas, este pipeline implementa un algoritmo SCD (Slowly Changing Dimension) Tipo 2 (Figura \ref{fig:dim_estudiante}).
...
El pipeline de centro encapsula lógicas de limpieza para la geolocalización analítica. El campo \textit{direccion} se particiona mediante \textit{FieldSplitter}, se limpia el prefijo literario de los códigos postales (\textit{ReplaceString}) y se formatea al tipo numérico (\textit{SelectValues}) antes de generar la clave \textit{id\_centro} (Figura \ref{fig:dim_centro}).
...
El flujo \textit{Full} requiere volcar en RAM todo el historial autonómico, algo insostenible para recargas periódicas. La variante \textit{Incremental} (Figuras \ref{fig:fact_rendimiento_anual_incremental} y \ref{fig:fact_rendimiento_anual_incremental_join}) resuelve esto leyendo solo las notas del \textit{\$\{CURSO\_ACTUAL\}}.
...
Los pipelines son secuenciados por flujos de trabajo (\textit{Workflows}). La orquestación sigue una arquitectura jerárquica encabezada por \textit{main.hwf} (Figura \ref{fig:main_workflow}), que delega en dos sub-workflows:

\begin{itemize}
    \item \textbf{\textit{dimensions.hwf}:} Extrae datos a \textit{Staging} y luego lanza en paralelo la carga de todas las dimensiones para maximizar el rendimiento (Figura \ref{fig:dimensions_workflow}).
    \item \textbf{\textit{facts.hwf}:} Ejecuta secuencialmente las tablas de hechos. Un nodo \textit{Simple Evaluation} inspecciona el parámetro dinámico \textit{\$\{CURSO\_ACTUAL\}}: si está vacío, enruta hacia el pipeline \textit{Full}; si contiene un curso, enruta hacia el \textit{Incremental} (Figura \ref{fig:facts_workflow}).
\end{itemize}
