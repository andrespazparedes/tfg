...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{imaxes/postgres-dwh_hop_conn.png}
...
Como se aprecia en la Figura \ref{fig:extract_all_to_staging}, el pipeline \textit{extract\_all\_to\_staging.hpl} establece 14 hilos independientes de tipo \textit{Table Input} (origen) conectados a \textit{Table Output} (destino). Este componente implementa un mecanismo de extracción incremental mediante el uso del parámetro dinámico \textit{\$\{CURSO\_ACTUAL\}}. Los nodos de extracción ejecutan subconsultas SQL que filtran y capturan exclusivamente los datos del curso lectivo en vigor (ej. \textit{WHERE curso\_academico = '\$\{CURSO\_ACTUAL\}'}). Para permitir recargas históricas completas, se incluye la condición lógica \textit{OR '\$\{CURSO\_ACTUAL\}' = ''}, garantizando una lectura masiva si el orquestador omite el parámetro. Este diseño evita volcados completos innecesarios y minimiza drásticamente la carga de red en cada ejecución por evaluación.
...
\begin{enumerate}
    ...
    \item \textbf{Carga Incremental tipo Upsert (\textit{Insert / update}):} Superada la lógica de transformación, el pipeline utiliza un nodo transaccional \textit{Insert / update} para volcar los datos en el esquema destino. Este componente evalúa en tiempo real si la combinación de claves ya existe en la tabla de hechos. Si existe, ejecuta una actualización (\textit{Update}) sobre las métricas; si es un registro inédito, realiza una inserción limpia (\textit{Insert}). Este mecanismo asegura la persistencia del historial acumulado y es el pilar que da soporte a la arquitectura \textit{Change Data Capture (CDC)} de la solución.
\end{enumerate}
...
Este pipeline (ver Figuras \ref{fig:fact_rendimiento_anual_full_p1} y \ref{fig:fact_rendimiento_anual_full_p2}) abandona la clásica consolidación estática anual y adopta una arquitectura de "Suma Acumulada" manteniendo el grano de la tabla a nivel de Evaluación (\textit{id\_estudiante} $\times$ \textit{id\_tiempo}).
...
\begin{enumerate}
    ...
    \item \textbf{Cálculo del Indicador de Abandono (\textit{Calculator}):} Como culminación, un nodo sumatorio simple (\textit{ADD3}) consolida matemáticamente el \textit{riesgo\_academico}, el \textit{riesgo\_adaptacion} y los puntos socioeconómicos. Esto arroja el indicador definitivo \textit{riesgo\_abandono}, que finalmente es almacenado en la tabla \textit{dwh.fact\_rendimiento\_anual} a través de un nodo masivo \textit{Table Output}.
\end{enumerate}

\subsubsection{Adaptaciones del pipeline Incremental}

El principal problema arquitectónico del flujo \textit{Full} es que, para calcular la penalización por fracasos previos, necesita volcar en la memoria RAM el historial completo de calificaciones de toda la comunidad autónoma. En una carga inicial esto es necesario, pero en una recarga periódica de evaluación resulta insostenible por el enorme consumo de recursos y tiempo. Para resolver esta ineficiencia matemática, se diseñó la variante \textit{fact\_rendimiento\_anual\_incremental.hpl} (ver Figura \ref{fig:fact_rendimiento_anual_incremental}), que aplica las siguientes soluciones:

\begin{enumerate}
    \item \textbf{Ingesta Parametrizada y Recálculo del Curso:} El nodo \textit{Table Input} inyecta la variable dinámica \textit{\$\{CURSO\_ACTUAL\}}, leyendo exclusivamente las calificaciones del ciclo lectivo en vigor. Se opta por recalcular el curso actual completo (en lugar de inyectar solo una única evaluación) para poder procesar y solventar automáticamente cualquier rectificación en las notas producida antes del cierre de actas, manteniendo un volumen de datos mínimo comparado con la carga histórica total.
    \item \textbf{Recuperación del Historial (\textit{Database Join}):} Al no disponer del bagaje histórico en RAM, el nodo \textit{Group By} (Suma Acumulada) se vuelve inoperante a nivel interanual. Para solucionarlo, un nodo \textit{Database Join} interroga directamente a la tabla destino (\textit{fact\_rendimiento\_anual}) filtrando por el año anterior del alumno (ver detalle en la Figura \ref{fig:fact_rendimiento_anual_incremental_join}). El objetivo clave es recuperar su \textit{riesgo\_academico} ya consolidado. Dado que el modelo matemático del riesgo es puramente aditivo, el sistema calcula de forma aislada los puntos de los suspensos del año actual y simplemente los suma al riesgo acumulado extraído de la base de datos.
    \item \textbf{Detección de Repetidores Dinámica:} La sub-rama analítica basada en LAG 1 se descarta al carecer del año previo en memoria. El flujo aprovecha el cruce anterior con la propia tabla \textit{fact\_rendimiento\_anual} para consultar si el alumno arrastra repeticiones previas (\textit{num\_repetir\_1\_2\_pri} y \textit{num\_repetir\_resto}), integrando estas penalizaciones en la fórmula actual de manera directa.
    \item \textbf{Carga no Destructiva (\textit{Insert / update}):} El nodo final abandona el volcado masivo y destructivo (\textit{Table Output}). Utiliza en su lugar el componente transaccional \textit{Insert / update}, permitiendo inyectar los nuevos cálculos de riesgo en el Data Warehouse mientras preserva intacto todo el historial de los años anteriores.
\end{enumerate}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{imaxes/fact_rendimiento_anual_full_p1.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{imaxes/fact_rendimiento_anual_full_p2.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{imaxes/fact_rendimiento_anual_incremental.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{imaxes/fact_rendimiento_anual_incremental_join.png}
...
\begin{enumerate}
    \item \textbf{\textit{dimensions.hwf}:} Este flujo se encarga de la extracción inicial y la carga de dimensiones (ver Figura \ref{fig:dimensions_workflow}). Su ejecución comienza disparando el pipeline de extracción a \textit{Staging}. Una vez los datos están en bruto en el esquema \textit{raw}, el workflow lanza en \textbf{paralelo} la carga de todas las dimensiones (\textit{dim\_tiempo}, \textit{dim\_curso}, \textit{dim\_estudiante}, etc.) para maximizar el rendimiento. Es crítico que todas finalicen con éxito para garantizar la integridad referencial.
    \item \textbf{\textit{facts.hwf}:} Tras asegurar las dimensiones, \textit{main.hwf} dispara este sub-workflow secuencial (ver Figura \ref{fig:facts_workflow}). Primero ejecuta el flujo atómico de calificaciones. Si finaliza con éxito, un nodo \textit{Simple Evaluation} inspecciona el valor del parámetro \textit{\$\{CURSO\_ACTUAL\}}. Si la variable está vacía, el orquestador enruta la ejecución hacia el pipeline de rendimiento \textit{Full} (carga masiva en memoria). Si contiene un curso específico, enruta dinámicamente hacia el pipeline \textit{Incremental} (arquitectura híbrida base de datos-memoria), garantizando la máxima eficiencia operativa por evaluación.
\end{enumerate}

\subsection{Propagación de parámetros dinámicos}

Una de las características más potentes de la orquestación implementada es la propagación jerárquica de variables. El proyecto evita el uso de valores "quemados" (\textit{hardcoded}) en código, utilizando en su lugar el parámetro global \textit{CURSO\_ACTUAL} para gobernar el comportamiento de carga (Full o Incremental).

El ciclo de vida de este parámetro fluye en cascada o \textit{top-down}:
\begin{enumerate}
    \item \textbf{Inyección Externa:} El parámetro se define en el momento de la ejecución desde fuera del contenedor (por ejemplo, a través de variables de entorno en el \textit{docker-compose} o argumentos del CLI de \textit{Hop Run}).
    \item \textbf{Recepción en el Workflow Principal:} El flujo \textit{main.hwf} está configurado para declarar y capturar este valor en sus propiedades de ejecución, registrándolo en la memoria del orquestador.
    \item \textbf{Transmisión a Sub-workflows:} A través de la configuración \textit{"Pass parameter values to sub-workflow"}, el \textit{main} inyecta el curso actual hacia abajo, entregándolo como contexto válido a \textit{dimensions.hwf} y \textit{facts.hwf}.
    \item \textbf{Consumo en los Pipelines:} Finalmente, los pipelines atómicos recogen la variable dinámica (invocada como \textit{\$\{CURSO\_ACTUAL\}}) y la inyectan directamente en el código SQL de los nodos \textit{Table Input} para filtrar fechas, o la evalúan lógicamente en pasos de enrutamiento.
\end{enumerate}
